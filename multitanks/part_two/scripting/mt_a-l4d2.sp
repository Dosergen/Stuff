...
public Plugin myinfo =
{
	...
	url = "https://forums.alliedmods.net/showthread.php?t=303729"
}
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	{
		strcopy(error, err_max, "[MT] This plugin is for Left 4 Dead 2 only.");
...
void OnFinaleEvents(Event event, const char[] name, bool dontBroadcast)
{
	if (!bMTOn)
		return;
...
void OnTankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	if (!bMTOn)
		return;
	int tank = GetClientOfUserId(event.GetInt("userid"));
...
Action CheckFrustration(Handle timer, any userid)
{
...
int MTListHandler(Menu menu, MenuAction action, int param1, int param2)
{
