...
public Plugin myinfo =
{
	...
	author = "Tordecybombo, FuzzOne, TacKLER, Dosergen",
...
public void OnPluginStart()
{
	...
	hEnabled = CreateConVar("l4d2_ais_enabled", "1", "[0=OFF|1=ON] Disable/Enable functionality of the plugin", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hDisableInVersus = CreateConVar("l4d2_ais_disable_in_versus", "1", "[0=OFF|1=ON] Automatically disable plugin in versus mode", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hFasterResponse = CreateConVar("l4d2_ais_fast_response", "0", "[0=OFF|1=ON] Disable/Enable faster special infected response", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hFasterSpawn = CreateConVar("l4d2_ais_fast_spawn", "0", "[0=OFF|1=ON] Disable/Enable faster special infected spawn (Enable when SI spawn rate is high)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hSafeSpawn = CreateConVar("l4d2_ais_safe_spawn", "0", "[0=OFF|1=ON] Disable/Enable special infected spawning while survivors are in safe room", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hSpawnWeights[SI_BOOMER] = CreateConVar("l4d2_ais_boomer_weight", "100", "The weight for a boomer spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnWeights[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_weight", "100", "The weight for a hunter spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnWeights[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_weight", "100", "The weight for a smoker spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnWeights[SI_TANK] = CreateConVar("l4d2_ais_tank_weight", "-1", "[-1 = Director spawns tanks] The weight for a tank spawning", FCVAR_NOTIFY, true, -1.0);
	hSpawnWeights[SI_CHARGER] = CreateConVar("l4d2_ais_charger_weight", "100", "The weight for a charger spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnWeights[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_weight", "100", "The weight for a jockey spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnWeights[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_weight", "100", "The weight for a spitter spawning", FCVAR_NOTIFY, true, 0.0);
	hSpawnLimits[SI_BOOMER] = CreateConVar("l4d2_ais_boomer_limit", "1", "The max amount of boomers present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_HUNTER] = CreateConVar("l4d2_ais_hunter_limit", "1", "The max amount of hunters present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SMOKER] = CreateConVar("l4d2_ais_smoker_limit", "1", "The max amount of smokers present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_TANK] = CreateConVar("l4d2_ais_tank_limit", "0", "The max amount of tanks present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_CHARGER] = CreateConVar("l4d2_ais_charger_limit", "1", "The max amount of chargers present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_JOCKEY] = CreateConVar("l4d2_ais_jockey_limit", "1", "The max amount of jockeys present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hSpawnLimits[SI_SPITTER] = CreateConVar("l4d2_ais_spitter_limit", "1", "The max amount of spitters present at once", FCVAR_NOTIFY, true, 0.0, true, 14.0);
	hScaleWeights = CreateConVar("l4d2_ais_scale_weights", "0", "[0=OFF|1=ON] Scale spawn weights with the limits of corresponding SI", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hWitchLimit = CreateConVar("l4d2_ais_witch_limit", "-1", "[-1 = Director spawns witches] The max amount of witches present at once (independant of l4d2_ais_limit).", FCVAR_NOTIFY, true, -1.0, true, 100.0);
	hWitchPeriod = CreateConVar("l4d2_ais_witch_period", "300.0", "The time (seconds) interval in which exactly one witch will spawn", FCVAR_NOTIFY, true, 1.0);
	hWitchPeriodMode = CreateConVar("l4d2_ais_witch_period_mode", "1", "The witch spawn rate consistency [0=CONSTANT|1=VARIABLE]", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	hSILimit = CreateConVar("l4d2_ais_limit", "3", "The max amount of special infected at once", FCVAR_NOTIFY, true, 1.0, true, float(MAX_INFECTED));
	hSpawnSize = CreateConVar("l4d2_ais_spawn_size", "1", "The amount of special infected spawned at each spawn interval", FCVAR_NOTIFY, true, 1.0, true, float(MAX_INFECTED));
	hSpawnTimeMode = CreateConVar("l4d2_ais_time_mode", "1", "The spawn time mode [0=RANDOMIZED|1=INCREMENTAL|2=DECREMENTAL]", FCVAR_NOTIFY, true, 0.0, true, 2.0);
	//hSpawnTimeFunction = CreateConVar("l4d2_ais_time_function", "0", "The spawn time function [0=LINEAR|1=EXPONENTIAL|2=LOGARITHMIC]", FCVAR_NOTIFY, true, 0.0, true 2.0);
	hSpawnTimeMin = CreateConVar("l4d2_ais_time_min", "0.0", "The minimum auto spawn time (seconds) for infected", FCVAR_NOTIFY, true, 0.0);
	hSpawnTimeMax = CreateConVar("l4d2_ais_time_max", "60.0", "The maximum auto spawn time (seconds) for infected", FCVAR_NOTIFY, true, 1.0);
...
void ConVarEnabled(ConVar convar, const char[] oldValue, const char[] newValue)
{
	EnabledCheck();
}

void ConVarFasterResponse(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SetAIDelayCvars();
}

void ConVarFasterSpawn(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SetAISpawnCvars();
}

void ConVarSafeSpawn(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SafeSpawn = hSafeSpawn.BoolValue;
}

void ConVarScaleWeights(ConVar convar, const char[] oldValue, const char[] newValue)
{
	ScaleWeights = hScaleWeights.BoolValue;
}

void ConVarSILimit(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SILimit = hSILimit.IntValue;
...
void ConVarSpawnSize(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SpawnSize = hSpawnSize.IntValue;
}

void ConVarSpawnTimeMode(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SpawnTimeMode = hSpawnTimeMode.IntValue;
...
void ConVarSpawnTime(ConVar convar, const char[] oldValue, const char[] newValue)
{
	if (!ChangeByConstantTime)
		SetSpawnTimes();
}

void ConVarSpawnWeights(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SetSpawnWeights();
...
void ConVarSpawnLimits(ConVar convar, const char[] oldValue, const char[] newValue)
{
	SetSpawnLimits();
}

void ConVarWitchLimit(ConVar convar, const char[] oldValue, const char[] newValue)
{
	WitchLimit = hWitchLimit.IntValue;
	if (WitchLimit < 0 && SpawnWeights[SI_TANK] >= 0)
	{
...
void ConVarWitchPeriod(ConVar convar, const char[] oldValue, const char[] newValue)
{
	WitchPeriod = hWitchPeriod.FloatValue;
	if (LeftSafeRoom && WitchLimit > 0)
		RestartWitchTimer(0.0); //restart timer after times change
...
void ConVarWitchPeriodMode(ConVar convar, const char[] oldValue, const char[] newValue)
{
	VariableWitchPeriod = hWitchPeriodMode.BoolValue;
	if (LeftSafeRoom && WitchLimit > 0)
		RestartWitchTimer(0.0); //restart timer after times change
...
Action SetConstantSpawnTime(int client, int args)
{
...
Action PresetWeights(int client, int args)
{
	char arg[16];
	GetCmdArg(1, arg, sizeof(arg));
...
Action kickbot(Handle timer, any client)
{
...
void EndWitchWaitTimer()
{
	...
	{
		if (hWitchWaitTimer != null)
		{
			delete hWitchWaitTimer;
			hWitchWaitTimer = null;
		}
		WitchWaitTimerStarted = false;
	}
}
...
Action StartSpawnTimerManually(int client, int args)
{
	if (Enabled)
	{
		if (args < 1)
...
Action SpawnWitchAuto(Handle timer, any waitTime)
{
	WitchTimerStarted = false;
	if (LeftSafeRoom)
	{
...
Action ResetSpawns(int client, int args)
{	
	KillSpecialInfected();
	if (Enabled)
	{
...
Action evtSurvivalStart(Event event, const char[] name, bool dontBroadcast)
{
	if (GameMode == 3)
	{  
		if (!LeftSafeRoom)
...
/*
void evtWitchHarasse(Event event, const char[] name, bool dontBroadcast)
{
	char names[32];
...
void evtWitchKilled(Event event, const char[] name, bool dontBroadcast)
{
	WitchCount--;
	if (WitchCountFull)
	{
