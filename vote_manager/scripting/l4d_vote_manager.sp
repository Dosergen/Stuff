...
Action VoteAction(int client, const char[] command, int argc)
{
	...
	if (argc == 1 && iVote[client] == Voted_CanVote && client != 0 && g_iVoteStatus == VOTE_POLLING)
	{
		char vote[5];
...
Action VoteStart(int client, const char[] command, int argc)
{
	...
	if ((ClientHasAccess(client, "cooldown_immunity") || g_fNextVote[client] <= flEngineTime) && g_iVoteStatus == VOTE_NONE)
	{
...
Action VoteResult(UserMsg msg_id, BfRead bf, const int[] players, int playersNum, bool reliable, bool init)
{
	...
	LogVoteManager("%T", votePassed ? "Vote Passed" : "Vote Failed", LANG_SERVER);
	VoteLogAction(-1, -1, votePassed ? "callvote (verdict 'passed')" : "callvote (verdict 'failed')");
	ClearVoteStrings();
	g_iVoteStatus = VOTE_NONE;
	return Plugin_Continue;
}
...
{
	bool passed = (StrEqual(name, "vote_passed"));
	LogVoteManager("%T", passed ? "Vote Passed" : "Vote Failed", LANG_SERVER);
	VoteLogAction(-1, -1, passed ? "callvote (verdict 'passed')" : "callvote (verdict 'failed')");
	ClearVoteStrings();
	g_iVoteStatus = VOTE_NONE;
}
...
{
	if (!IsValidVoteType(issuee)) // this plugin has no idea what this vote is, prevent them from running this vote.
	{
